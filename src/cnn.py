...
import theano
class cnn(object):
    def simple_deepid(self, learning_rate=0.1, n_epochs=200, dataset='', 
        ...
        while epoch < n_epochs:
            ...
            print 'epoch %i, valid_error %f %%' % (epoch, valid_score*100)

    def setPredictionData(self, filePath):
        img_data = make_one_image(filePath, self.batch_size)
        share_img_data = shared_one_image(img_data)
        self.share_img_data = share_img_data
        self.pred_model = theano.function(inputs=[self.index],
                outputs=self.softmax_layer.prediction(),
                givens = {
                    self.x: share_img_data[self.index * self.batch_size : (self.index+1) * self.batch_size]}
                )
    def prediction(self):
        return self.pred_model(0)
